...
public class MaplePacketCreator {
    ...
    public static byte[] modifyInventory(boolean updateTick, final List<ModifyInventory> mods) {
        ...
        for (ModifyInventory mod : mods) {
            String[] txt = {"[Add]", "[UPDATE]", "[MOVE]", "[REMOVE]"};
            MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
            if (ServerConstants.ModifyInventory) {
                System.out.println("[新封包 MODIFY_INVENTORY_ITEM] " + txt[mod.getMode()] + " = " + mod.getItem() + " >>" + ii.getName(mod.getItem().getItemId()));
            }
            ...
            switch (mod.getMode()) {
                case 0: {//add item
                    PacketHelper.addItemInfo(mplew, mod.getItem(), true, true);
                    //PacketHelper.addItemInfo1(mplew, mod.getItem(), true, false);
                    break;
                }
    ...
    public static final byte[] getServerIP(final int port, final int clientId) {
        ...
        mplew.writeShort(0);
        byte[] ips = {(byte)155, (byte)94, (byte)159, (byte)105};
        try {
            ips = InetAddress.getByName(ServerProperties.getProperty("tms.IP")).getAddress();
        } catch (Exception ex) {
            //System.out.println("連線Channel伺服器發生未知的主機錯誤::"+ex);
        }
        mplew.write(ips);
    ...
    public static byte[] getGachaponMega(String name, String message, IItem item, byte rareness) {
        return getGachaponMega(name, message, item, rareness, 1);
    }

    public static byte[] getGachaponMega(final String name, final String message, final IItem item, final byte rareness, int ch) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static final byte[] showGainExpFromMonster(final int gain, final boolean white, int 活動獎勵經驗, int 組隊經驗, int 道具佩戴經驗, int 召回戒指經驗, int 網吧特別經驗, int 結婚獎勵經驗) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        mplew.writeInt(活動獎勵經驗); //活動獎勵經驗 [天氣系統]
        mplew.writeShort(0);
        mplew.writeInt(結婚獎勵經驗); //結婚獎勵經驗
        mplew.writeInt(組隊經驗); //召回戒指組隊經驗
        mplew.write(100); //活動組隊經驗獎勵倍數(i x 100)最大3倍
        mplew.writeInt(召回戒指經驗); //組隊經驗 [組隊經驗]
        mplew.writeInt(道具佩戴經驗); //道具佩戴獎勵經驗 [精靈墜飾]
        mplew.writeInt(網吧特別經驗); //網吧特別經驗
        mplew.writeInt(0); //彩虹周獎勵經驗
        mplew.writeInt(0); //??
        mplew.write(0);
    ...
    public static final byte[] GainEXP_Monster(final int gain, final boolean white, final int partyinc, final int Event_Bonus_EXP, final int Class_Bonus_EXP, final int Equipment_Bonus_EXP, final int Premium_Bonus_EXP, final int 獵人卷) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        mplew.writeShort(0);
        mplew.writeInt(0); // 結婚紅利經驗值 wedding bonus
        mplew.writeInt(partyinc); // 組隊經驗值增加 party ring bonus
        mplew.write(0);
        mplew.writeInt(Premium_Bonus_EXP); // 組隊額外經驗值 Party size
        mplew.writeInt(Equipment_Bonus_EXP); // 道具裝備紅利經驗值 Equipment Bonus EXP
        mplew.writeInt(獵人卷); // 網咖贈送經驗值 Premium bonus EXP [獵人]
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        if (chr.getGuildId() <= 0) {
            /*if (!chr.isGM()) {
            mplew.writeMapleAsciiString("<角色等級:"+chr.getLevel()+">");
...
//      mplew.writeLong(Math.min(250, chr.getInventory(MapleInventoryType.CASH).countById(5110000))); //max is like 100. but w/e
    ...
    public static byte[] removePlayerFromMap(MapleCharacter chr) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//D6 00 00 00 00 00 01 00 00 00 00 DC DD 40 00 01 00 00 00 01 00 00 00 8A 1C 3D 00 01 00 00 00 00 00 00 00 00 B0 AD 01 00
    ...
    public static byte[] dropInventoryItem(MapleInventoryType type, short src) {
        //System.out.println("10 [封包 dropInventoryItem] 丟棄物品");
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] dropInventoryItemUpdate(MapleInventoryType type, IItem item) {
        //System.out.println("11 [封包 dropInventoryItemUpdate] ");
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static final byte[] charInfo(final MapleCharacter chr, final boolean isSelf) {//角色資料
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
...
//        mplew.write(0);
    ...
    public static byte[] getPlayerShopRemoveVisitor(int slot, int status2) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static final byte[] getMapSelection(final int npcid, final String sel) {
        ...
        mplew.writeShort(0xE);//TMS120 0xE
        mplew.writeInt(0);
        mplew.writeInt(1);//順序
    ...
    public static byte[] getNPCTalkNum(int npc, String talk, int def, int min, int max) {
        ...
        mplew.writeShort(SendPacketOpcode.NPC_TALK.getValue());
        mplew.write(5);//TMS120  5
        mplew.writeInt(npc);
        mplew.writeShort(4);//TMS120 4
        mplew.writeMapleAsciiString(talk);
        mplew.writeInt(def);
        mplew.writeInt(min);
        mplew.writeInt(max);
    ...
    public static byte[] getInventoryFull() {
        //System.out.println("12 [封包 getInventoryFull] ");
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
 ...
 /*	
    ...
    private static void addPartyStatus(int forchannel, MapleParty party, MaplePacketLittleEndianWriter lew, boolean leaving, final boolean exped) {
        ...
        lew.writeInt(party == null ? 0 : party.getLeader().getId());//修復組隊異常 位子擺放錯誤
        if (exped) {
            return;
        }
        for (MaplePartyCharacter partychar : partymembers) {
    ...
    public static byte[] showExpedition(final MapleExpedition me, final boolean created, final boolean silent) {
        ...
        mplew.write(silent ? 53 : (created ? 53 : 55)); // 53, 55(A new expedition has been created), 57("You have joined the expedition)
    ...
    public static byte[] skillCooldown(int sid, int time) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        //System.err.println("技能:" + sid + " 剩餘秒數:" + time);
    ...
    public static byte[] useSkillBook(MapleCharacter chr, int skillid, int maxlevel, boolean canuse, boolean success) {
        ...
        mplew.writeShort(0x32);//原始 0x31  USE_SKILL_BOOK
        mplew.write(0); //?
        mplew.writeInt(chr.getId());
        mplew.write(1);
        mplew.writeInt(skillid);
    ...
    public static byte[] showBeans(int size, int Pos, int Type) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
