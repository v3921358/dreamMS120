...
public class ReactorScriptManager extends AbstractScriptManager {
    ...
    public final void act(final MapleClient c, final MapleReactor reactor) {
        try {
            final Invocable iv = getInvocable("reactor/" + reactor.getReactorId() + ".js", c);
             c.getPlayer().dropMessage(5, "找到 Reactor 文件中的 " + reactor.getReactorId() + ".js 文件.");
            if (iv == null) {
                return;
            }
            ...
            System.err.println("執行Reactor文件出錯 ReactorID: " + reactor.getReactorId() + ", ReactorName: " + reactor.getName() + ":" + e);
            FileoutputUtil.log(FileoutputUtil.ScriptEx_Log, "執行Reactor文件出錯 ReactorID: " + reactor.getReactorId() + ", ReactorName: " + reactor.getName() + ":" + e);
        }
    }
    ...
    public final List<ReactorDropEntry> getDrops(final int rid) {
        ...
        try {
            ...
            System.err.println("從SQL中讀取箱子的爆率出錯.箱子的ID " + rid + e);
            return ret;
        } finally {
            try {
                if (rs != null) {
