...
public void Autoexp(int exp, int meso, int cash, int item, int itemamount) { 
 
Collection<MapleCharacter> chrs = getChannelServer().getPlayerStorage().getAllCharacters(); 
...
int giveExp = chr.getLevel() * exp; 
 
int giveMeso = chr.getLevel() * meso;
 
int giveCash = ((rand(20, 25) % (20 - 10 + 1) + 10) * (rand(2, 3) % (3 - 2 + 1)));
if (chr.getLevel() >= 1 && ((chr.getMapId() >= 910000000 && chr.getMapId() <= 910000022) || chr.getMap
 
().isTown())) { 
chr.gainExp(giveExp, true, true, false); 
chr.gainMeso(giveMeso, true); 
chr.modifyCSPoints(1, giveCash, true); 
chr.gainItem(item, itemamount); 
 
// chr.saveToDB(true, true); 
 
chr.dropMessage(-1, "掛網領取獎勵 " + giveExp + " 經驗 " + giveMeso + " 楓幣 " + giveCash + " 點數");
