...
import client.CoinPlayerHandler;
...
public abstract class AbstractPlayerInteraction {
    ...
    public void worldMessageItem(String message, IItem item) {
        if (getClient().getPlayer().isGM()) {
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.itemMegaphone(message, false, getClient().getChannel(), item));
        } else {
            World.Broadcast.broadcastSmega(MaplePacketCreator.itemMegaphone(message, false, getClient().getChannel(), item));
        }
    }
    ...
    public final void openNpc(final int id, String npcMode) {
        NPCScriptManager.getInstance().start(getClient(), id, 0, npcMode);
    }
    ...
    public void gainPetItem(int itemid) {
        gainPetItem(itemid, (short) 1, 45);
    }

    public void gainPetItem(int itemid, short quantity, int period) {
        MapleInventoryManipulator.addById(getClient(), itemid, quantity, "", MaplePet.createPet(itemid, MapleInventoryIdentifier.getInstance()), period);
    }
    ...
    public boolean getTempFlag(final int flag) {
     ...
     public void useDice(int useamount) {
        c.getPlayer().useDice(useamount);
    }

    public int getDiceNum() {
        return c.getPlayer().getDiceNum();
    }
    ...
    public final void gainMaplePoint(final int amount) {
        ...
        return CoinPlayerHandler.getInstance().getBPoints(getClient());
    }
    public void setBPoints(int money){
        CoinPlayerHandler.getInstance().setBPoints(money,getClient());
    }
      public int getCoin(){
        return CoinPlayerHandler.getInstance().getCoin(getClient());
    }
    public void setCoin(int money){
        CoinPlayerHandler.getInstance().setCoin(money,getClient());
    }
    public int getCoinAcc(){
        return CoinPlayerHandler.getInstance().getCoinAcc(getClient());
    }
    public void setCoinAcc(int money){
        CoinPlayerHandler.getInstance().setCoinAcc(money,getClient());
    }
}
