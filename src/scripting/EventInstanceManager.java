...
public class EventInstanceManager {
    ...
    public final void modifyCSPoints(final int quantity) {
        for (MapleCharacter chr : getPlayers()) {
            chr.modifyCSPoints(2, quantity, true);
            chr.dropMessage(-1, "完成所有關卡額外獲得獎勵.");
        }
    }
    ...
    public final void saveBossQuest(final int points) {
        ...
        for (MapleCharacter chr : getPlayers()) {
            /*int item[][] = {{20, 20, 2}, {40, 20, 4}, {500, 50, 10}, {1500, 150, 30}};
            for (int[] a : item) {
                if (a[0] == points) {
                    chr.modifyCSPoints(2, a[1], true);
                    chr.gainItem(4032056, a[2], "BSPQ獲得");
                    chr.dropMessage(-1, "完成所有關卡額外獲得獎勵.");
                }
            }*/
