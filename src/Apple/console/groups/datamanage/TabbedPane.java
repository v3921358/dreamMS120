...
public abstract class TabbedPane extends WebPanel {
    ...
    protected Component getSearchGroup() {
        ...
        GroupPanel groupPanel1 = new GroupPanel(GroupingType.fillFirst, searchTextField, searchButton),
                groupPanel2 = new GroupPanel(10, new WebCheckBox("顯示搜索結果", isSelectedSearchResult()) {
                    {
    ...
    protected Component getCenterComponent() {
        ...
        if (showEditColumn()) {
            operationPane.add(new WebButton("設置列", Start.loadIcon("table.png")) {
                {
                    setRolloverDecoratedOnly(true);
                    setDrawFocus(false);
                    addActionListener(new AbstractAction() {
                        ...
                        public void actionPerformed(ActionEvent e) {
                            WebDialog dialog = new WebDialog(owner, "設置顯示列名", true);
    ...
    protected java.util.List<Action> getActions(boolean isIDTable) {
        ...
        ret.add(getDefaultActionListener("刪除", Start.loadIcon("off.png"), isIDTable));
        ret.add(getDefaultActionListener("編輯", Start.loadIcon("edit.png"), isIDTable));
        return ret;
    }
...
//        WebOptionPane.showMessageDialog(this, "該功能正在調整中,目前僅支持查詢.", "警告", 0);
        ...
        if (!isIDTable && idTable.getSelectedRow() == -1) {
            WebOptionPane.showMessageDialog(this, "請先在左側選擇要添加分類名稱", "警告", 0);
            return;
        }
        ...
        WebButton button_ok = new WebButton(addtext) {
            {
                addActionListener(e -> {
                    ...
                    addToChangeList(isIDTable ? DataMamageMode.ID_編輯 : DataMamageMode.DATA_編輯, id, data);
                    dialog.dispose();
                });
            }
...
//        WebOptionPane.showMessageDialog(this, "該功能正在調整中,目前僅支持查詢.", "警告", 0);
        ...
        if (WebOptionPane.showConfirmDialog(this, "確定要刪除 " + rowcount + " 條記錄嗎？", "警告", JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
            int[] rows = table.getSelectedRows();
            int i = 0;
            DefaultTableModel model = (DefaultTableModel) table.getModel();
            for (int row : rows) {
                Vector<String> vector = (Vector<String>) model.getDataVector().get(row - i);
                addToChangeList(isIDTable ? DataMamageMode.ID_刪除 : DataMamageMode.DATA_刪除, getSelectedId(), vector);
    ...
    private class TableMouseAdapter extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (!isIDTable && e.getClickCount() == 2) {
                editAction((WebTable) e.getSource(), false, false);
            }
        }
