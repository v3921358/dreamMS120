...
//	showdown = 100;
    ...
    private final void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte Class_Bonus_EXP_PERCENT, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
           ...
           //if (attacker.getMapId() != 220080001)  
            // 伺服器經驗值倍數加成
            exp = (int) Math.min(Integer.MAX_VALUE, exp * (attacker.getLevel() < 10 ? GameConstants.getExpRate_Below10(attacker.getJob()) :attacker.getEXPMod() * ChannelServer.getInstance(map.getChannel()).getExpRate()));
            if(attacker.getGuildId() != 0)
            {
                if(MapleGuildWar.isLord(attacker.getGuild().getId()))
                    exp *= 2;
            }
            // 給角色幸運獵人經驗
            int 幸運獵人 = (int) (attacker.getStat().expBuff / 100.0);

            attacker.gainExpMonster(exp, true, highestDamage, numExpSharers, pty, Premium_Bonus_EXP_PERCENT, 幸運獵人, this);
        }
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean venom, final boolean checkboss, final int delay) {
        if (!isAlive()) {
            return;
        }
                    ...
                    if (attacker.getKey().getLevel() - partychar.getLevel() <= 5 || stats.getLevel() - partychar.getLevel() <= 5) {
                        ...
                        if (pchr != null) {
                            if (pchr.isAlive() && pchr.getMap() == map) {
                                ...
                                if (pchr.getStat().equippedWelcomeBackRing && Premium_Bonus_EXP < 80) {
    ...
    //設定召換怪物者ID
    public void setSpawnChrid(final int id){
        stats.setSpawnChrId(id);
    }
}
