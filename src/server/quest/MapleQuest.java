...
public class MapleQuest implements Serializable {
    ...
    public void start(MapleCharacter c, int npc) {
        if (c.isAdmin()) {
            c.dropMessage(6, "開始任務 start: " + npc + " autoStart：" + autoStart + " checkNPCOnMap: " + checkNPCOnMap(c, npc) + " canStart: " + canStart(c, npc));
        }
    ...
    public boolean complete(MapleCharacter c, int npc) {
        return complete(c, npc, null);
    }

    public boolean complete(MapleCharacter c, int npc, Integer selection) {
        if ((autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
            ...
            c.getClient().sendPacket(MaplePacketCreator.showSpecialEffect(10)); // Quest completion
            c.getMap().broadcastMessage(c, MaplePacketCreator.showSpecialEffect(c.getId(), 10), false);
        } else if (id == 29507 || id == 22402) { //修正任務沒辦法完成 [22402 和龍騎士見面]
            for (MapleQuestAction a : completeActs) {
                if (!a.checkEnd(c, selection)) {
                    return false;
                }
    ...
    private boolean checkNPCOnMap(MapleCharacter player, int npcid) {
        //mir = 1013000
        return (GameConstants.isEvan(player.getJob()) && (npcid == 1013000 || npcid == 1063018)) || (player.getMap() != null && player.getMap().containsNPC(npcid));
    }
