        ...
        switch (onFirstUserEnter.fromString(scriptName)) {
            ...
            case StageMsg_together: {
                switch (c.getPlayer().getMapId()) {
                    case 103000800:
                        c.getPlayer().getMap().startMapEffect("隊員們去找克魯特, 除掉他所要求的鱷魚個數, 並收集優惠券吧!", 5120017);
                        break;
                    case 103000801:
                        c.getPlayer().getMap().startMapEffect("去找可以通往下一階段的門的3條線, 然後纏著吧!", 5120017);
                        break;
                    case 103000802:
                        c.getPlayer().getMap().startMapEffect("去找可以通往下一階段的門的3個腳板吧.", 5120017);
                        break;
                    case 103000803:
                        c.getPlayer().getMap().startMapEffect("上桶，揭開正確的組合！", 5120017);
                        break;
                    case 103000804:
                        c.getPlayer().getMap().startMapEffect("除掉超級綠水靈吧!", 5120017);
                        break;
                }
                break;
            }
