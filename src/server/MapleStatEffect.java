...
//    private List<Pair<Integer, Integer>> randomMorph;
    ...
    private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill, final boolean overTime, final byte level) {
        ...
        ret.mhpR = (byte) MapleDataTool.getInt("mhp_temp", source, 0);//HP BUFF
        ret.mmpR = (byte) MapleDataTool.getInt("mmp_temp", source, 0);//MP BUFF
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        if (applyfrom.skillisCooling(sourceid)) {
            if (!skill) {
                return false;
            }
        }
                        ...
                        if (targetMapId != 12 && charMapId != 10) {
                            if (targetMapId != 10 && charMapId != 12) {
                                if (targetMapId != charMapId) {
                                    System.err.println("玩家 " + applyto.getName() + " 嘗試回到一個非法的位置 (" + applyto.getMapId() + "->" + target.getId() + ")");
                                    return false;
                                }
                            }
                        }
