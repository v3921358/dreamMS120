...
public class Item implements IItem, Serializable {
    ...
    private int equipOnlyId = -1; //裝備道具的唯一ID
    ...
    public IItem copy() {
        ...
        ret.equipOnlyId = equipOnlyId; //裝備道具的唯一ID
        return ret;
    }
    ...
    public int getEquipOnlyId() {
        return equipOnlyId;//獲取裝備道具的唯一ID
    }
    ...
    public void setEquipOnlyId(final int OnlyId) {
        ...
        for (ChannelServer cserv : ChannelServer.getAllInstances()) {
            for (MapleCharacter mch : cserv.getPlayerStorage().getAllCharacters()) {
                player = cserv.getPlayerStorage().getCharacterByName(mch.getName());
                if (OnlyId != -1 && player.isGM() && sum == 1) {
                    //player.dropMessage(5, "[設定唯一ID] Item: " + id + " EquipOnlyId: " + OnlyId);
                }
                sum += 1;
            }
        }
        this.equipOnlyId = OnlyId;//設定道具的唯一ID
    }
