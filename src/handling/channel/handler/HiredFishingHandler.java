...
public class HiredFishingHandler {
    ...
    public static final void UseHiredFishing(final LittleEndianAccessor slea, final MapleClient c) {
            ...
            switch (state) {
                ...
                case 0:
                    ...
                    if (fishing == null && pack == null) {
                        c.sendPacket(PlayerShopPacket.sendFishBox());//精靈小釣手說明
                    } else if (fishing == null && pack != null) {
                        if (pack.getItems().size() <= 0) {
                            if (!check(c.getPlayer(), pack)) {//空間不足
                                c.sendPacket(PlayerShopPacket.merchItem_Message((byte) 0x21));
                                return;
                            }
                            if (deletePackage(c.getPlayer().getId(), c.getPlayer().getAccountID(), pack.getPackageid())) {//刪除
        ...
        if (fishing != null) {
            ...
            if (pack == null) {
                ...
                if (!check(c.getPlayer(), pack)) {
                    c.sendPacket(PlayerShopPacket.merchItem_Message((byte) 0x22));
                    return;
                }
    ...
    public static final void FishingItemStore(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer() == null) {
            return;
        }
    ...
    private static final boolean check(final MapleCharacter chr, final MerchItemPackage pack) {
        ...
        if (chr.getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() <= eq
            ...
            String msg = "[空間不足]";
            msg += (eq > 0 ? " 裝備欄:" + eq : "") + (use > 0 ? " 消耗欄:" + use : "") + (setup > 0 ? " 裝飾欄:" + setup : "") + (etc > 0 ? " 其他欄:" + etc : "") + (cash > 0 ? " 特殊欄:" + cash : "");
