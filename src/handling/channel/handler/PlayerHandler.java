...
public class PlayerHandler {
    ...
    public static final void ChangeKeymap1(final LittleEndianAccessor slea, final MapleCharacter chr) {
        if (slea.available() > 8 && chr != null) { // else = pet auto pot
            chr.updateTick(slea.readInt());
            final int numChanges = slea.readInt();
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getPlayerFishing() != null) {
            chr.dropMessage(5, "無法與精靈小釣手同時進行");
        } else if (map == 749050500 || map == 749050501 || map == 749050502) {
    ...
    public static final void CancelChair(final short id, final MapleClient c, final MapleCharacter chr) {
        if (id == -1) { // Cancel Chair
            final int map = chr.getMapId();
            if (/*itemId == 3011000*/chr.getPlayerFishing() == null && (map == 749050500 || map == 749050501 || map == 749050502)) {
    ...
    public static final void TakeDamage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (ServerConstants.受傷訊息) {
            chr.dropMessage(-5, "[玩家受傷] 受傷類型: " + type + " 受傷數值: " + damage);
        }
        ...
        if (damage > 0) {
            ...
            if (ServerConstants.受傷訊息) {
                chr.dropMessage(-5, "[玩家傷害] 最終受到傷害 " + damage);
            }
        }
        if (!chr.isHidden()) {
    ...
    public static final void CancelBuffHandler(final int sourceid, final MapleCharacter chr) {
        ...
        if (skill.isChargeSkill()) {
            ...
            chr.cancelEffect(skill.getEffect(1), false, -1);//上線隱身
        }
    }
    ...
    public static final void ThrowSkill(final LittleEndianAccessor slea, final MapleClient c) {
     ...
     * 8500003 - 小黑水雷
     */
    public static final void Monster_Bomb(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (skill != null) {
            if (monster != null) {
                ...
                c.sendPacket(MaplePacketCreator.skillCooldown(skill.getId(), 50000));
...
//	slea.skip(5); // unknown
        ...
        if (res != null && c.getPlayer().getMap() != null) { // TODO more validation of input data
            ...
            if(GameConstants.isGuildPVPMap(chr.getMapId()) && chr.getStat().getHp() <= 0){
                GuildWarDeath(chr);
            }
...
//	    }
    ...
    public static void GuildWarDeath(MapleCharacter chr) {
        int size = 0;
        int GuildID = 0;
        MapleGuildWar MapleWar = World.GuildWar.getGuildWar((MapleGuildWar.isPVPMap(chr.getMapId())));
        MapleGuildWar war = MapleGuildWar.getInstance();
        if (GameConstants.getAllWarStatus()) {
            for (int i = 0; i < MapleWar.PartyGuild.length; i++) {
                if (MapleWar.PartyGuild[i][0] == chr.getGuildId()) {
                    MapleWar.PartyGuild[i][1]--;
                    if (MapleWar.PartyGuild[i][1] == 0) {
                        for (int j = 0; j < MapleWar.PartyGuild.length; j++) {
                            if (MapleWar.PartyGuild[j][1] >= 1) {
                                size++;
                                GuildID = MapleWar.PartyGuild[j][0];
                            }
                        }
                        if (size == 1) {
                            MapleWar.EndTime(MapleWar.getVillage(), MapleWar.getId(), chr.getClient(), GuildID);
                            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "公會戰-" + MapleWar.getVillage() + "10秒後即將結束,勝利的公會是[" + war.getGuildName(GuildID) + "]"));
                            break;
                        }
                    }
                    break;
...
//            slea.skip(6); //D3 75 00 00 00 00
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (!wheel) {
                    ...
                    //c.sendPacket(MTSCSPacket.useWheel((chr.getInventory(MapleInventoryType.CASH).countById(5510000) - 1)));
