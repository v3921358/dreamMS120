...
public class NPCHandler {
    ...
    public static List<String> QuestOnly = new ArrayList();
    ...
    public static final void QuestAction(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) {
            ...
            case 2: { // Complete Quest
                final int npc = slea.readInt();
                chr.updateTick(slea.readInt());
                if (QuestOnly.contains(chr.getId() + "_" + quest)) {
                    chr.dropMessage(-5, "[任務提醒]已發現重複任務 Quest: " + quest + " ID: " + chr.getName());
                    System.err.println("[任務提醒]已發現重複任務 Quest: " + quest + " ID: " + chr.getName());
                    break;
                } else {
                    QuestOnly.add(chr.getId() + "_" + quest);
                }
                boolean complete = false;
                if (slea.available() >= 4) {
                    complete = q.complete(chr, npc, slea.readInt());
                } else {
                    complete = q.complete(chr, npc);
                }
                //任務完成才給予獎勵
                if (complete) {
                    chr.modifyCSPoints(2, 5);
                    chr.dropMessage(-5, "完成任務: 給予獎勵 5 點楓葉點數");
                    if (chr.isAdmin()) {
                        chr.dropMessage(6, "完成系統任務 NPC: " + npc + " Quest: " + quest);
                    }
                } else {
                    QuestOnly.remove(chr.getId() + "_" + quest);
                    chr.dropMessage(5, "任務尚未完成，請確認是否條件不足或空間不夠。");
                }
            ...
            case 5: { // Scripted End Quest
                final int npc = slea.readInt();
                if (QuestOnly.contains(chr.getId() + "_" + quest)) {
                    chr.dropMessage(-5, "[腳本提醒]已發現重複任務 Quest: " + quest + " ID: " + chr.getName());
                    System.err.println("[腳本提醒]已發現重複任務 Quest: " + quest + " ID: " + chr.getName());
                    if (chr.getQuestStatus(quest) == 1) {
                        chr.dropMessage(-5, "[腳本提醒]已發現尚未完成任務 Quest: " + quest + " ID: " + chr.getName() + " 已自動替你解除");
                        QuestOnly.remove(chr.getId() + "_" + quest);
                    }
                    break;
                } else {
                ...
                boolean complete = false;
                complete = NPCScriptManager.getInstance().endQuest(c, npc, quest, false);
                //任務完成才給予獎勵
                if (!complete) {
                    QuestOnly.remove(chr.getId() + "_" + quest);
                    chr.dropMessage(5, "[腳本]任務尚未完成，請確認是否條件不足或空間不夠。");
                    FileoutputUtil.log("任務腳本.ini", FileoutputUtil.output_text(c, chr, " 職業:" + chr.getJob(), " 等級:" + chr.getLevel(), " 任務:" + q.getName() + "(" + quest + ") 時間:" + FileoutputUtil.CurrentReadable_Time()));
                    break;
                }
                c.sendPacket(MaplePacketCreator.showSpecialEffect(10)); // Quest completion
