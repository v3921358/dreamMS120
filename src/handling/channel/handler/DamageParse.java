    ...
    public static void applyAttack(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
        ...
        for (final AttackPair oned : attack.allDamage) {
            ...
            if (monster != null) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (fixeddmg != -1) {
                        ...
                        if (monsterstats.getOnlyNoramlAttack()) {
                            ...
                            // System.err.println("[傷害: " + eachd + ", 預計傷害: " + maxDamagePerHit + ", 怪物ID: " + monster.getId() + "] [職業: " + player.getJob() + ", 等級: " + player.getLevel() + ", 技能: " + attack.skill + "]");
                            //FileoutputUtil.log("Logs/Plugin/"+player.getName()+".ini", "[傷害: " + eachd + ", 預計傷害: " + maxDamagePerHit + ", 怪物ID: " + monster.getId() + "] [職業: " + player.getJob() + ", 等級: " + player.getLevel() + ", 技能: " + attack.skill + "]");
                            if (Tempest) { // Monster buffed with Tempest
                                ...
                                if (eachd > maxDamagePerHit * 3) {
                                    ...
                                    if (eachd > maxDamagePerHit * 5) {
                                        ...
                                        //player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2); //傷害太高
                                    }
                                }
                            } else {
                ...
                //處理進階隱身術機率計算
                boolean 影身術 = true;
                final Skill divine = (Skill) SkillFactory.getSkill(4330001);
                if (player.getSkillLevel(divine) > 0 && player.getBuffedValue(MapleBuffStat.DARKSIGHT) != null && !player.skillisCooling(divine.getId())) {
                    final MapleStatEffect divineShield = divine.getEffect(player.getSkillLevel(divine));
                    if (Randomizer.nextInt(100) < divineShield.getProb()) {
                        影身術 = false;
                    }
                }
...
//                            monster.handleSteal(player);
        ...
        if (attack.skill == 4331003 && totDamageToOneMonster < hpMob) {
         ...
         if(player.getClient().getChannel() == PVP_Channel && GameConstants.isGuildPVPMap(player.getMapId())){
            if(!World.GuildWar.getGuildWar(MapleGuildWar.isPVPMap(player.getMapId())).getPVPWaitTime()){
                MaplePvP.doPvP(player,player.getMap(),attack); //PVP攻擊
            }
        }
        if (attack.skill != 0 && (attack.targets > 0 || (attack.skill != 4331003 && attack.skill != 4341002)) && attack.skill != 21101003 && attack.skill != 5110001 && attack.skill != 15100004 && attack.skill != 11101002 && attack.skill != 13101002) {
            effect.applyTo(player, attack.position);
                                ...
                                if (player.getClient().getChannel() == PvpSettings.channel && PvpSettings.usepvp) {
        ...
        if (totDamage > 1) {
            ...
            if (tracker.getAttacksWithoutHit() > 50) {//攻擊特效無聲音
            ...
            if (monster != null) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (fixeddmg != -1) {
                        ...
                        if (player.isAdmin() && eachd > 0) {
                            player.dropMessage(-1, "魔法攻擊打怪傷害 : " + eachd + " 服務端預計傷害 : " + maxDamagePerHit + " 是否超過 : " + (eachd > maxDamagePerHit) + " 是否超過封鎖 : " + (eachd > maxDamagePerHit*2));
                        }
                        if (monsterstats.getOnlyNoramlAttack()) {
