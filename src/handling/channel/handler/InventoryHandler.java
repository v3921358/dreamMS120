...
public class InventoryHandler {
    ...
    public static final void UseItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit()) || chr.getMapId() == 610030600) { //cwk quick hack
            /* if (toUse.getItemId() == 2450000) {
                Accounts_HunterExp(chr, false);
            }*/
    ...
    /*
    //紀錄幸運獵人使用時間
    public static int Accounts_HunterExp(final MapleCharacter chr, boolean dc) {
        //System 顯示
        System.err.print("dc狀態: " + dc);
        Connection con = DatabaseConnection.getConnection();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT * FROM accounts_hunterexp WHERE cid = ? and name = ?");
            ps.setInt(1, chr.getId());
            ps.setString(2, chr.getName());
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                if (dc) {
                    PreparedStatement psu = con.prepareStatement("UPDATE accounts_hunterexp SET dc = ? WHERE cid = ? and name = ?");
                    psu.setInt(1, 1);
                    psu.setInt(2, chr.getId());
                    psu.setString(3, chr.getName());
                    psu.executeUpdate();
                    psu.close();
                    System.err.print(" 更新斷線SQL, ");
                } else if (!dc) {
                    long First = -1, Last = -1;
                    if (rs.getInt("dc") == 1) {
                        First = rs.getLong("expireFirst");
                        Last = System.currentTimeMillis();
                        if (Last - First < 0) {
                            rs.close();
                            ps.close();
                            PreparedStatement psu = con.prepareStatement("DELETE FROM accounts_hunterexp WHERE cid = ? and name = ?");
                            psu.setInt(1, chr.getId());
                            psu.setString(2, chr.getName());
                            psu.executeUpdate();
                        System.err.print(" 沒有數值刪除, "); 
                        }
                    }
                    PreparedStatement psu = con.prepareStatement("UPDATE accounts_hunterexp SET expireFirst = ?, dc = 0 WHERE cid = ? and name = ?");
                    psu.setLong(1, System.currentTimeMillis());
                    psu.setInt(2, chr.getId());
                    psu.setString(3, chr.getName());
                    psu.executeUpdate();
                    psu.close();
                    System.err.print(" 更新使用時間\r\n");
                    return (int) (1800000 - (Last - First));
                }
            } else {
                if (!dc) {
                    PreparedStatement psu = con.prepareStatement("INSERT INTO accounts_hunterexp (cid, name, expireFirst, expireLast, dc) values (?, ?, ?, ?, ?)");
                    psu.setInt(1, chr.getId());
                    psu.setString(2, chr.getName());
                    psu.setLong(3, System.currentTimeMillis());
                    psu.setLong(4, -1);
                    psu.setInt(5, 0);
                    psu.executeUpdate();
                    psu.close();
                    System.err.print(" 建立新的SQL\r\n");
                }
            }
            rs.close();
            ps.close();
 ...
 /*
    ...
    public static final void UseMagnify(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eqq.getState() == 1 && (magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12) || (magnify.getItemId() == 2460001 && reqLevel <= 7) || (magnify.getItemId() == 2460000 && reqLevel <= 3))) {
            ...
            if (c.getPlayer().isAdmin()) {
                c.getPlayer().dropMessage(-5, "鑒定裝備: 放大鏡: " + magnify.getItemId() + " insight: " + eqSlot + " eqq: " + eqq);
            }
            c.getPlayer().forceUpdateItem(eqq, true);
            //c.sendPacket(MaplePacketCreator.scrolledItem(magnify, eqed ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP, toReveal, false, true));
    ...
    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c, final MapleCharacter chr, final int vegas) {
        ...
        if (scrollSuccess == IEquip.ScrollResult.CURSE) {
            ...
            //c.sendPacket(MaplePacketCreator.scrolledItem(scroll, MapleInventoryType.EQUIP, toScroll, true, false));
            if (dst < 0) {
            ...
            //c.sendPacket(MaplePacketCreator.scrolledItem(scroll, MapleInventoryType.EQUIP, scrolled, false, false));
        }
        //更新背包
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            switch (toUse.getItemId()) {
                ...
                case 2430112: //miracle cube
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        if (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430112) >= 25) {
                            ...
                            c.getPlayer().dropMessage(5, "10個奇幻方塊碎片才可以兌換1個奇幻方塊.");
                        }
                    } else {
                        c.getPlayer().dropMessage(5, "請空出一些欄位。");
                    }
                ...
                case 2430229: //bunny rickshaw 60 day
                    ...
                    expiration_days = 60;
                    break;
                case 2430049: //紀念冒險家歸來的禮品箱
                    int itemid[][] = {{2030002, 25}, {2030003, 25}, {2030004, 25}, {2030005, 25}, {2030019, 25}, {2000002, 300}, {2000006, 300}, {2120000, 100}};
                    if (c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= itemid.length) {
                        for (int[] id : itemid) {
                            MapleInventoryManipulator.addById(c, id[0], (short) id[1]);
                        }
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2430049, 1, true, false);
                    } else {
                        c.getPlayer().dropMessage(5, "請空出" + itemid.length + "個欄位。");
                    }
                    break;
                case 2430032: //黑色袋子
                    int Eq[] = {1004219, 1052789, 1072957, 1082598, 1004224, 1052794, 1072962, 1082603, 1302315, 1302314, 1312185, 1312184, 1322236, 1322235, 1402236, 1402235, 1412164, 1412163, 1422171, 1422170, 1432200, 1432199, 1442254, 1442253, 1004220, 1052790, 1072958, 1082599, 1004225, 1052795, 1072963, 1082604, 1372207, 1372206, 1382245, 1382244, 1004221, 1052791, 1072959, 1082600, 1004226, 1052796, 1072964, 1082605, 1452238, 1452237, 1462225, 1462224, 1004222, 1052792, 1072960, 1082601, 1004227, 1052797, 1072965, 1082606, 1332260, 1332259, 1472247, 1472246, 1004223, 1052793, 1072961, 1082602, 1004228, 1052798, 1072966, 1082607, 1482202, 1482201, 1492212, 1492211};
                    if (c.getPlayer().getSpace(1) < 1) {
                        c.getPlayer().dropMessage(5, "你的空間不足請在裝備欄位空出一格。");
                    } else {
                        MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 1, true, false);
                        int eqitem = Eq[Randomizer.nextInt(Eq.length)];
                        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                        Equip equip;
                        equip = ii.randomizeStats((Equip) ii.getEquipById(eqitem));
                        short SDIL = 0;
                        if (equip.getStr() > 0) {
                            equip.setStr((short) SDIL); //力量
                        }
                        if (equip.getDex() > 0) {
                            equip.setDex((short) SDIL); //敏捷
                        }
                        if (equip.getInt() > 0) {
                            equip.setInt((short) SDIL); //智力
                        }
                        if (equip.getLuk() > 0) {
                            equip.setLuk((short) SDIL);
                        }
                        if (equip.getMatk() > 0) {
                            equip.setMatk((short) 5);
                        }
                        if (equip.getStr() > 0) {
                            equip.setMdef((short) 10);
                        }
                        if (equip.getWatk() > 0) {
                            equip.setWatk((short) 5);
                        }
                        if (equip.getWdef() > 0) {
                            equip.setWdef((short) 10);
                        }
                        if (equip.getHp() > 0) {
                        ...
                        equip.setUpgradeSlots((byte) SDIL);
                        MapleInventoryManipulator.addFromDrop(c, equip, false);
                        c.sendPacket(MaplePacketCreator.getShowItemGain(eqitem, (short) 1, true));
                    }
                    break;
                case 2430051: //被封印的箱子
                    c.getSession().writeAndFlush(tools.MaplePacketCreator.enableActions());
                    NPCScriptManager.getInstance().start(c, 9900007, 0, "VIP_chr");
                    break;
            }
        }
        if (mountid > 0) {
    ...
    public static boolean UseSummonBagNotCSP(int itemId){
    ...
    public static final void UseSummonBag(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            ...
            if (c.getPlayer().isGM() || !FieldLimitType.SummoningBag.check(chr.getMap().getFieldLimit())) {
                ...
                if(UseSummonBagNotCSP(toUse.getItemId()))
                {
                    boolean result = false;
                    for (IItem item : c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).list()) {
                        int equipId = item.getItemId();
                        if(equipId == 1112023)
                           result = true;
                    }
                    if(result == true )
                    {
                        if(!chr.getRectMonster())
                        {
                            chr.startReincarnat();
                        }else{
                            if(chr.getReincarnation()){
                        ...
                        chr.dropMessage("請確認自己是否有裝備輪迴石碑");
                        c.sendPacket(MaplePacketCreator.enableActions());
                        return;
                    }
                }
    ...
    public static final void UseTreasureChest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            ...
            if (GameConstants.gachaponRareItem(item.getItemId()) > 0) {
                if (box == "金寶箱") {
                    if (chr.isGM()) {
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.getGachaponMega("[" + box + "] " + c.getPlayer().getName(), " : 從" + box + "中獲得", item, (byte) 2));
                    } else {
                        World.Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega("[" + box + "] " + c.getPlayer().getName(), " : 從" + box + "中獲得", item, (byte) 2));
                    }
                } else {
                    if (chr.isGM()) {
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.itemMegaphone("[" + box + "] " + c.getPlayer().getName() + " : 從" + box + "中獲得", true, c.getChannel(), item));
                    } else {
                        World.Broadcast.broadcastMessage(MaplePacketCreator.itemMegaphone("[" + box + "] " + c.getPlayer().getName() + " : 從" + box + "中獲得", true, c.getChannel(), item));
                    }
                }
            }
        } else {
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5561000: { // 任意門高級卷
                boolean allow = false;
                if (c.getPlayer().getLevel() >= 30 && itemId == 5560000) {
                    allow = true;
                } else if (c.getPlayer().getLevel() >= 70 && itemId == 5561000) {
                    allow = true;
                }
                if (!allow) {
                    c.getPlayer().dropMessage(5, "您的等級不足，必須達到 " + (itemId == 5560000 ? "30" : "70") + " 等以上");
                    break;
                }
                slea.skip(1);
                ...
                if (FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())) {
                    c.getPlayer().dropMessage(5, "你所在的區域無法使用");
                } else if (FieldLimitType.VipRock.check(target.getFieldLimit())) {
                    c.getPlayer().dropMessage(5, target.getMapName() + " 被禁止傳送");
                } else if (c.getPlayer().inMapleLand()) {
            ...
            case 2320000: // 瞬移之石(專屬道具, 無法交換)
            case 5041000: // 高級瞬移之石
            case 5040000: // 瞬移之石
            case 5040001: { // 可樂翅膀
                c.getPlayer().dropMessage(5, "此道具已關閉.");
                if (false) {
                    break;
                }
            ...
            case 5062000: { //奇幻魔方
                ...
                if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                    ...
                    if (eq.getState() >= 5) {
                        ...
                        //c.sendPacket(MaplePacketCreator.scrolledItem(toUse, MapleInventoryType.EQUIP, item, false, true));
            ...
            case 5074000: { // Skull Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(13, c.getChannel(), sb.toString(), ear));
...
//                }
            ...
            default:
                ...
                UseSkillBook(slot, itemId, c, c.getPlayer()); //this should handle removing*/
                    ...
                    case 533:
                        ...
                        break;
                    default:
                        System.err.println("使用未處理的商城道具 : " + itemId + " 封包：" + slea.toString(true));
                        break;
                }
                break;
        }
        ...
        if (cc) {
            if (!c.getPlayer().isAlive() || c.getPlayer().getEventInstance() != null || FieldLimitType.ChannelSwitch.check(c.getPlayer().getMap().getFieldLimit())) {
                c.getPlayer().dropMessage(1, "更換通道失敗！");
                return;
            }
            c.getPlayer().dropMessage(5, "正在更改頻道請稍候。");
    ...
    public static final void Pickup_Player(final LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (mapitem.getMeso() > 0) {
                ...
                MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                final String Pick_items = "物品處理/撿起/" + c.getPlayer().getName() + ".ini";
                final String id_Name = mapitem.getItem().getItemId() + "x" + mapitem.getItem().getQuantity() + "-" + ii.getName(mapitem.getItem().getItemId());
                FileoutputUtil.log(Pick_items, FileoutputUtil.output_text(c, chr, " 撿起道具:" + id_Name, " 唯一ID:" + mapitem.getItem().getEquipOnlyId()));
            }
        } finally {
            lock.unlock();
        }
    ...
    private static final boolean getIncubatedItems(MapleClient c) {
        if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < 2 || c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() < 2 || c.getPlayer().getInventory(MapleInventoryType.SETUP).getNumFreeSlot() < 2) {
            c.getPlayer().dropMessage(5, "請確保你有足夠的背包空間.");
            return false;
        }
    ...
    public static boolean UseSkillBook(byte slot, int itemId, MapleClient c, MapleCharacter chr) {
        Item toUse = (Item) chr.getInventory(GameConstants.getInventoryType(itemId)).getItem(slot);
        ...
        while (true) {
            ...
            int job = (int) Math.floor(CurrentLoopedSkillId / 10000);
            if (job != chr.getJob() && !(job >= 430 && job <= 434)) { //偵測自己職業才以取得
                continue;
            }
            ...
            if (CurrSkillData != null && CurrSkillData.canBeLearnedBy(chr.getJob()) && (ReqSkillLevel == null || chr.getSkillLevel(CurrSkillData) >= ReqSkillLevel) && chr.getMasterLevel(CurrSkillData) < MasterLevel) {
                ...
                if (SuccessRate == null || Randomizer.nextInt(100) <= SuccessRate) {
                    ...
                    chr.changeSkillLevel(skill2, chr.getSkillLevel(skill2), (byte) (int) MasterLevel);
                } else {
                    success = false;
                }
    ...
    public static boolean UseSkillBook1(byte slot, int itemId, MapleClient c, MapleCharacter chr) {
        Item toUse = (Item) chr.getInventory(GameConstants.getInventoryType(itemId)).getItem(slot);
        if (toUse == null || toUse.getQuantity() < 1 || toUse.getItemId() != itemId/* || chr.hasBlockedInventory()*/) {
            return false;
        }
        Map<String, Integer> skilldata = MapleItemInformationProvider.getInstance().getSkillStats(toUse.getItemId());
        Map<String, Integer> skillids = MapleItemInformationProvider.getInstance().getSkillStats(toUse.getItemId());
        if (skilldata == null) { // Hacking or used an unknown item
            return false;
        }
        boolean canuse = false, success = false;
        ...
        Integer ReqSkillLevel = skilldata.get("reqSkillLevel");
        Integer MasterLevel = skilldata.get("masterLevel");
        System.err.printf("%d  %d %d", SuccessRate, ReqSkillLevel, MasterLevel);

        int i = 0;
        Integer CurrentLoopedSkillId;
        while (true) {
            CurrentLoopedSkillId = skillids.get(String.valueOf(i));
            System.err.printf(" %d", CurrentLoopedSkillId);
            i++;
            if (CurrentLoopedSkillId == null || MasterLevel == null) {
                break; // End of data
            }
    ...
    public static final void UseSkillBook(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        for (;;) {
            ...
            if (Math.floor(CurrentLoopedSkillId / 10000) == chr.getJob()) { //偵測自己職業才以取得
