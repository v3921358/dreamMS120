...
function action(mode, type, selection) {
	...
	if (cm.getJob() == 510 || cm.getJob() == 520) {
	    ...
	    cm.sendNext("你想成為海盜嗎？如果要成為我們中的一員，您將需要符合我們的標準。我需要你 #b達到最低等級 10， 敏捷 在 20 以上。#k 讓我們來看看...");
	} else {
	    if (selection == 0) {
		cm.sendNext("你不想感受海上的自由嗎？你不想要權力，名氣和其他一切嗎？那麼你應該加入我們，並享受它。");
		cm.dispose();
	    } else if (selection == 1) {
		status = 99;
		cm.sendNext("你準備好了吧？現在試圖忍受我的攻擊2分鐘。我不會對你輕鬆的祝你好運，因為你需要它。");
	    }
	}
    } else if (status == 2) {
	if (cm.getPlayerStat("LVL") >= 10) {
	    requirements = true;
	    cm.sendYesNo("很好，你準備好成為我們中的一員嗎？");
	} else {
	    cm.sendNextPrev("嗯...我不認為你已經訓練夠了，但是。當你變得更強壯的時候見我");
	}
    } else if (status == 3) {
	if (requirements)
	    cm.sendNext("歡迎來到海盜樂團！你可能首先需要花一些時間作為流浪漢，但是更好的日子肯定會早日亮相，比你想像的早！同時，讓我和你分享一些我的能力。");
	else
	    cm.dispose();
    } else if (status == 4) {
	if (cm.getJob() == 0) {
