...
function action(mode, type, selection) {
	...
	if (sel == 1) {
	    ...
	    if (cm.canHold(2300001,120) && cm.haveItem(5350000,1)) {
			if (!cm.haveItem(2300001, 120 * (Rate-1)) && !cm.haveItem(2300001, 120 * (Rate-1), false, false)) {
	...
	if (sel == 1) {
	    if (cm.canHold(2300000,120) && cm.getMeso() >= 300000) {
			if (!cm.haveItem(2300000, 120 * Rate + 1) && !cm.haveItem(2300000, 120 * (Rate + 1), false, false)) {
				cm.gainMeso(-300000);
