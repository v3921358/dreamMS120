...
function action(mode, type, selection) {
	...
	status--;
    if (status == 0) {
	if (!(cm.getJob() == 310 || cm.getJob() == 320)) {//非職業
	    cm.sendOk("你好！找我有什麼事情嗎");
	    cm.dispose();
	    return;
	}
	if ((cm.getJob() == 310 || cm.getJob() == 320) && cm.getPlayerStat("LVL") >= 70 && cm.getPlayerStat("RSP") <= (cm.getPlayerStat("LVL") - 70) * 3) {
	    if (cm.getQuestStatus(100100) == 1){ //開始
		cm.warp(100000201);
		cm.sendOk("回去找#b赫麗娜#k. 他會幫助你.");
		cm.dispose();
            } else if (cm.getQuestStatus(100101) == 2 && cm.haveItem(4031057)) { //完成
                cm.sendNext("噢噢...看樣子順利完成#b赫麗娜#k給你的任務了。我就相信你能夠辦到。但你應該沒有忘記還有第二階段的考驗吧？在進行第二階段的考驗之前，先把項鍊交給我吧。");
            } else if (cm.getQuestStatus(100102) == 1) { //開始
		cm.warp(211040401, 0);
                cm.sendOk("在#b艾納斯島 - 冰原雪域#k內找到#r神聖的石頭#k，且發現裡面的神聖石頭");
                cm.dispose();
	    } else {
		cm.sendYesNo("\r你好！找我有什麼事情嗎？哦...你想進行三轉，成為更強的#b弓箭手#k事嗎？當然用我的力量可以讓你更強，但是在這之前我必須了解你付出了多少的努力。到現在為止來找我的年輕人很多，而能夠實際證明自己確實很強大的勇者卻沒有幾個...怎麼樣？就算很困難你也要試試看嗎？");
	    }
	} else {
